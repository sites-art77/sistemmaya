...
(function(){
  ...
  function authGateHtml(){
    ...
    return `<div class="maya-login-card"><div class="maya-login-mark">M</div><h1>MAYA Garden</h1><p class="maya-login-sub">Acesso online protegido</p><form onsubmit="event.preventDefault();cloudAuthSignIn()"><label class="font-bold text-sm">Usuário<input id="auth-username" class="maya-input mt-1" autocomplete="username" placeholder="admin1"></label><label class="font-bold text-sm mt-3">Senha<input id="auth-password" type="password" class="maya-input mt-1" autocomplete="current-password" placeholder="sua senha"></label><button class="maya-btn w-full mt-4" type="submit">Entrar no sistema</button></form><p class="maya-login-status">${escCloud(status)}</p><p class="maya-login-note">É necessário estar conectado à internet para usar o sistema.</p></div>`;
  }
